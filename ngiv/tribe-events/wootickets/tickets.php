		...
		foreach ( $tickets as $ticket ) {
			...
			if ( $ticket->date_in_range( current_time( 'timestamp' ) ) ) {
				...
				if ( $product->is_in_stock() ) {
					...
					echo '<span class="tickets_nostock">' . esc_html__( 'Sorry, this class is full! Please click the button below to be added to the waitlist.', 'event-tickets-plus' ) . '</span>' . '<a class="btn btn-primary btn-block enabled" href="mailto:clases@nginyc.com">Join Waitlist</a>';
				}
				echo '</td>';
			...
			<tr>
				<td colspan="4" class="woocommerce add-to-cart">
					...
					<button type="submit" name="wootickets_process" value="1"
					        class="button alt"><?php esc_html_e( 'Attend', 'event-tickets-plus' );?></button>
				</td>
			</tr>
			<?php
		} ?>
